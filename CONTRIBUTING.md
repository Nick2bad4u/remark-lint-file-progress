...
Use a `type/description` branch and the emoji/bracket commit convention in `.github/agent-commit-message-instructions.md`. Changes should include evidence that Markdown, trees, diagnostics, and exit codes remain unchanged.

On Windows, reproduce the console regression in a real PowerShell terminal with the code below. It tests the packed artifact on Node 22.0.0 with both host-version profiles, both output streams, Unicode filenames, ANSI colors, and shutdown summaries. The probe checks that reporter write methods and terminal columns stay unchanged. Inspect the output for intact `élève-東京.md`, spinner frames, and colored marks; the original code page is restored afterward.

```powershell
$progressEncoding = [Console]::OutputEncoding
try {
    [Console]::OutputEncoding = [Text.Encoding]::GetEncoding(437)
    npm run test:compat:windows-console
} finally {
    [Console]::OutputEncoding = $progressEncoding
}
```
...
The ESLint 3.4.0 and remark 2.2.7 inspector bundles reapply root-relative favicon URLs after rendering. `scripts/finalize-inspector-assets.mjs` prefixes those known icon references with this site's deployment base. The remark bundle also references a missing `remark/logo-square.svg`; the script supplies that alias from its bundled favicon while leaving an upstream-supplied asset intact. Documentation checks reject these root-relative startup icons and require the alias. Remove these compatibility steps when the inspectors handle both cases themselves.

The same finalizer creates HTML entrypoints for each inspector's declared static routes. GitHub Pages does not provide a nested SPA fallback, so these entrypoints support refreshing or directly opening routes such as `/remark-inspector/rules`. The documentation gate requires an entrypoint for every declared route.
