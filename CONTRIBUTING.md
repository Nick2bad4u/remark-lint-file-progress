...
The remark inspector 2.2.7 bundle references `remark/logo-square.svg` but does not ship it. `scripts/finalize-remark-inspector.mjs` supplies that alias from the inspector's bundled favicon when missing, and the documentation checks require all referenced startup icons. Remove this compatibility step when the inspector supplies the asset itself. It leaves an upstream-supplied icon intact.
