...
Throttling limits displayed paths, not observed counts. Compact mode advances a generic activity spinner for each eligible event on interactive terminals and emits one generic notice when redirected. `ttyOnly` suppresses both progress and summaries when the selected stream is not a terminal, including when `showSummaryWhenHidden` is enabled.
